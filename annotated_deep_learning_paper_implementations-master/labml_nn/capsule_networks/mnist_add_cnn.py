...
Lets try to use CNN baselines (checking, compare, fusion etc) )
...
class MNISTCNNNetworkModel(Module):
    """
...
# class Configs(MNISTConfigs, SimpleTrainValidConfigs):
    ...
    def step(self, batch: Any, batch_idx: BatchIndex):
        ...
        loss = self.antispoof_loss(pred, target)
...
def cnn_network_model(c: Configs):
    """Set the model"""
    return MNISTCNNNetworkModel().to(device)
...
def cnntest():
    ...
    with torch.no_grad():
        ...
        if cc>0:
            ...
            features = (classifier(faces_t.to(device))).cpu().numpy()
            result_pred.append(features)
            print("cc: ",cc, "faces_pred:", features)
...
def ML_antispoof():
    ...
    # feature_test_path='/home/evgeniy/models/efficientnet/test_features.pkl'
    ...
    for ii in range(10, 100):
        ...
        #
        # print("first 200 acc: ",accuracy_score(Y_test, y_pred))
